 ...
 */
class ElementTextTests extends GrailsUnitTestCase {
	...
	void testLoad() {
		...
		def entity = generator.getElementText()
		assertNotNull "Save not successful", entity.save()
		def loaded = ElementText.load(entity.uri)
	...
	void testLoadAll() {
		...
		loompService.clear()
		def entities = generator.getElementTexts(NUM).sort { it.uri }
		assertEquals entities.size(), NUM
		entities.each { assertNotNull "Save not successful", it.save() }

		def loaded = ElementText.loadAll()
		assertFalse "no elements retrieved", loaded.isEmpty()
		assertEquals "number of loaded elements is different", entities.size(), loaded.size()
	...
	void testLoadAllLimit() {
		...
		def loaded = ElementText.loadAll(params)
		assertFalse "no elements retrieved", loaded.isEmpty()
		assertEquals "number of loaded elements is different", loaded.size(), LIMIT
	...
	void testLoadAllOffset() {
		...
		def loaded = ElementText.loadAll(params)
		assertFalse "no elements retrieved", loaded.isEmpty()
		assertEquals "number of loaded elements is different", loaded.size(), (entities.size() - OFFSET)
	...
	void testLoadAllPaging() {
		...
		while (loaded.size() > 0) {
			...
			loaded = ElementText.loadAll(params)
		}
	}
	...
	void testLoadAllSorted() {
		...
		loompService.clear()
		def entities = generator.getElementTexts(NUM).unique { it.title }.sort { it.title }
		entities.each { assertNotNull "Save not successful", it.save() }

		def params = [sort: DC.title]
		def loaded = ElementText.loadAll(params)
		assertFalse "no elements retrieved", loaded.isEmpty()
		assertEquals "number of loaded elements is different", loaded.size(), entities.size()
		assertEquiv(entities, loaded)
	}
