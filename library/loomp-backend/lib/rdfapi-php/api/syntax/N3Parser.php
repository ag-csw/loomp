...
function str2unicode_nfc($str=""){
  ...
  function getSpan($list, $start, $end) {
	if (!is_numeric($start) || !is_numeric($end)) trigger_error("getSpan: invalid params: start=$start, end=$end");
...
//var_dump($newres[0]);
  ...
  function getPrefixes($list) {
    ...
    while (($pos = array_search('@prefix',$list)) !== false) {
		list($binding, $list) = $this->getSpan($list, $pos, ($pos+4));
		$prefixes[$binding[$ns_i]] = substr($binding[$name_i],1,-1);
		$this->parsedNamespaces[substr($binding[$name_i],1,-1)] = substr($binding[$ns_i],0,-1);
    }
